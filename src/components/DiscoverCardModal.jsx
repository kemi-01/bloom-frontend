...
const handleDiscover = async () => {
  if (!writeUp.trim()) {
    alert("Your profile will not appear in Discover until you add a write-up.");
    return;
  }
...
const handleClearDiscover = async () => {
  if (!window.confirm("Are you sure you want to remove yourself from Discover?")) return;

  setUploading(true);

  try {
    await axios.delete(`${API_URL}/api/users/discover`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("bloomToken")}`,
      },
    });
  } catch (err) {
    // If it's 404 (already cleared), ignore
    if (err.response?.status !== 404) {
      console.error("Clear discover error:", err.response?.data || err.message);
      alert("Failed to clear discover info");
      return;
    }
  } finally {
    ...
    setCountry("");
    setPronouns("");
    onSave(null); // Tell parent that discover info is gone
    onClose();
    setUploading(false);
  }
};
...
return (
  ...
  <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50"> <div className="bg-[#111] rounded-2xl p-6 w-full max-w-md relative max-h-[90vh] overflow-y-auto shadow-lg animate-slide-up"> <h2 className="text-white text-xl font-bold mb-4 text-center">
